...
const openCreateAdminModalBtn = document.getElementById(
  "openCreateAdminModalBtn",
);
...
const createAdminSuccessModal = document.getElementById(
  "createAdminSuccessModal",
);
const closeCreateAdminSuccessBtn = document.getElementById(
  "closeCreateAdminSuccess",
);
...
function isEmployee() {
  return ["employe", "employé", "employee", "user"].includes(
    normalizeRole(currentUser?.role),
  );
}
...
async function initProfilePage() {
  ...
  const roleLabel = formatRoleLabel(
    currentUser.role || currentUser.poste || "Utilisateur",
  );
  ...
  if (avatar) {
    avatar.textContent = formatAvatar(
      currentUser.nom || currentUser.name || currentUser.prenom || "U",
    );
  }
...
async function loadTaskStats() {
  ...
  const assigned = tasks.filter(
    (task) => Number(task.id_responsable) === userId,
  ).length;
  const done = tasks.filter((task) => {
    ...
    return (
      status === "terminé" &&
      (isSuperAdmin() ||
        Number(task.id_createur) === userId ||
        Number(task.id_responsable) === userId)
    );
  }).length;
...
async function loadAdminUsers() {
  try {
    ...
    adminRecords = users.filter((user) => {
      ...
      return ["administrateur", "admin", "superadmin", "super_admin"].includes(
        roleKey,
      );
    });
...
function renderAdminList() {
  ...
  const visibleAdmins = adminRecords
    .filter((user) => {
      const roleKey = normalizeRole(user.role);
      return roleKey === "administrateur" || roleKey === "admin";
    })
    .slice(0, 1);
...
function openAdminActionModal(action, admin) {
  ...
  if (actionDelete)
    actionDelete.classList.toggle("hidden", action !== "delete");
  ...
  createAdminForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showToast(
          response.message || "Erreur lors de la création de l’administrateur.",
          "error",
        );
      }
    } catch (error) {
...
function bindAdminActionModal() {
  ...
  deleteAdminBtn?.addEventListener("click", async () => {
    ...
    adminRecords = adminRecords.filter(
      (item) => Number(item.id) !== Number(selectedAdmin.admin.id),
    );
    renderAdminList();
    closeAdminModal();
  });
